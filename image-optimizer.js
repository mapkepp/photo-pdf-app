...
export class ImageOptimizer {
  ...
  dataURLToBlob(dataUrl) {
    ...
    if (parts.length !== 2) {
      console.error('❌ Некорректный формат Data URL');
      throw new Error('Некорректный формат Data URL');
    }
    const contentTypeMatch = parts[0].match(/:(.*?);/);
    if (!contentTypeMatch) {
      console.error('❌ Не удалось извлечь MIME-тип из Data URL');
      throw new Error('Не удалось извлечь MIME-тип');
    }
   ...
   * @returns {Promise<{width: number, height: number, size: number, type: string}>} — информация об изображении
   */
